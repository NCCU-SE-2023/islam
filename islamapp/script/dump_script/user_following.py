...
class UserFollowing(Document):
    ...
    def create(raw_data):
        ...
        if latest_user_following is not None:
            following_change_count = raw_data["following_count"] - latest_user_following.following_count
            if following_change_count == 0:
                following_change_list=["none"]
            else:
                following_add_list = list(set(raw_data["following_list"] ) - set(latest_user_following.following_list))
                following_minus_list = list(set(latest_user_following.following_list) - set(raw_data["following_list"] ))
                following_change_list.clear()
                following_change_list = ["added_"+ i for i in following_add_list] + ["removed_" + i for i in following_minus_list]
            ...
            if raw_data['scraped_task_id'] not in latest_user_following.scraped_task_list:
        ...
        userfollowing = UserFollowing(user_following_id=user_following_id, create_at=create_at, 
                                      scraped_ig_id=raw_data["scraped_ig_id"], following_count=raw_data["following_count"] , 
                                      following_list=raw_data["following_list"] , following_change_count=following_change_count,
    ...
    def get_latest_user_following_by_ig_id(scraped_ig_id: str):
        try:
            return UserFollowing.objects(scraped_ig_id=scraped_ig_id).order_by('-create_at').first()
    ...
    def delete_user_following_by_ig_id(scraped_ig_id: str):
        try:
            userFollower = UserFollowing.objects(scraped_ig_id=scraped_ig_id).order_by('-create_at').first()
            if userFollower:
                userFollower.delete()
                return 1
        except Exception as e:
