...
class UserFollowers(Document):
    ...
    @staticmethod
    def create_user_followers(raw_data):
        ...
        user_followers_id = md5(str(create_at).join(raw_data["scraped_ig_id"]).join(str(random.random())).encode()).hexdigest()
        latest_user_followers = UserFollowers.get_latest_user_followers_by_ig_id(raw_data["scraped_ig_id"])      
        ...
        if latest_user_followers is not None:
            followers_change_count = raw_data["followers_count"] - latest_user_followers.followers_count
            if followers_change_count == 0:
                followers_change_list=["none"]
            else:
                followers_add_list = list(set(raw_data["followers_list"] ) - set(latest_user_followers.followers_list))
                followers_minus_list = list(set(latest_user_followers.followers_list) - set(raw_data["followers_list"] ))
                followers_change_list.clear()
                followers_change_list = ["added_"+ i for i in followers_add_list] + ["removed_" + i for i in followers_minus_list]
            ...
            if raw_data['scraped_task_id'] not in latest_user_followers.scraped_task_list:
        ...
        userFollowers = UserFollowers(user_followers_id=user_followers_id, create_at=create_at, 
                                      scraped_ig_id=raw_data["scraped_ig_id"], followers_count=raw_data["followers_count"] , 
                                      followers_list=raw_data["followers_list"] , followers_change_count=followers_change_count,
                                      followers_change_list=followers_change_list, scrape_user_count=scrape_user_count,
    ...
    def get_latest_user_followers_by_ig_id(scraped_ig_id: str):
        try:
            return UserFollowers.objects(scraped_ig_id=scraped_ig_id).order_by('-create_at').first()
    ...
    def delete_user_followers_by_ig_id(scraped_ig_id: str):
        try:
            userFollower = UserFollowers.objects(scraped_ig_id=scraped_ig_id).order_by('-create_at').first()
            if userFollower:
                userFollower.delete()
                return 1
        except Exception as e:
